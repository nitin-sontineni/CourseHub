...
function Notes() {
  ...
  if (!mounted) {
    axios
      .post("http://localhost:8080/student/lecturelink", {
        courseId: window.sessionStorage.getItem("student_course_id"),
        lecNo: window.sessionStorage.getItem("student_lecNo"),
        studentId: window.sessionStorage.getItem("student_id"),
      })
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8080/student/note/add", {
        note: {
          courseId: window.sessionStorage.getItem("student_course_id"),
          lecNo: window.sessionStorage.getItem("student_lecNo"),
          studentId: window.sessionStorage.getItem("student_id"),
  ...
  function handleDelete(key) {
    axios
      .post("http://localhost:8080/student/note/delete", {
        noteId: key,
      })
      .then((res) => {
        axios
  ...
  function handleChecked(key, timeStamp, content, check) {
    axios
      .post("http://localhost:8080/student/note/update", {
        note: {
          courseId: window.sessionStorage.getItem("student_course_id"),
          lecNo: window.sessionStorage.getItem("student_lecNo"),
          studentId: window.sessionStorage.getItem("student_id"),
  ...
  return (
    <div>
      ...
      <div className="notes-app">
        <Box>
          {todos.length === 0 ? (
            <h3 style={{ color: "white" }}> No Notes Added</h3>
          ) : (
            <Box
              sx={{
                margin: 1,
            ...
            >
              {todos.map((elem) =>
                elem.studentId === window.sessionStorage.getItem("student_id") ? (
                  <Box sx={{ margin: 1 }}>
                    <Card sx={{ width: 670 }} id={elem._id}>
                      <CardContent>
                        <Box
                          ...
                          >
                            {!elem.edit ? (
                              <Typography
                                variant="h7"
                                color="text.primary"
                                style={{ display: "flex", justifyContent: "flex-start", alignContent: "flex-start" }}
                              >
                                {elem.content}
                              </Typography>
                            ) : (
                      ...
                      <CardActions style={{ display: "flex", justifyContent: "flex-end" }}>
                        {!elem.edit ? (
                          <BorderColorIcon onClick={() => handleEdit(elem._id)} />
                        ) : (
                          <CheckIcon onClick={() => handleUpdate(elem._id, elem.timeStamp)} />
                        )}
